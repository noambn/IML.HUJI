...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # raise NotImplementedError()
        m = X.shape[0]
        inner_product = np.inner(X, self.weights)
        sigmoid = 1 / (1 + np.exp(-inner_product))
        to_sum = y * inner_product - np.log(sigmoid)
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # raise NotImplementedError()
        m = X.shape[0]
        inner_product = X @ self.weights
        exponent = np.exp(inner_product) / (1 + np.exp(inner_product))
        return - (y @ X - X.T @ exponent) / m
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        # raise NotImplementedError()
        regularizetion = self.regularization_module_.compute_jacobian(**kwargs)
        if self.include_intercept_:
            regularizetion = np.insert(regularizetion, 0, 0)
        return self.fidelity_module_.compute_jacobian(
            **kwargs) + self.lam_ * regularizetion
